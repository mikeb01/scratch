...
public class ColumnStoreRiskRepository implements RiskRepository
{
    ...
    private void resizeTo(int length)
    {
        ...
        if (openQuantity == null)
        {
            ...
            margin = new double[length];
            unrealisedPnL = new double[length];
            valuation = new double[length];
    ...
    {
        return round(getCell(cumulativeCost, accountId));
    }

    public long getProfit(long accountId)
    {
        return round(getCell(realisedPnL, accountId));
    }

    public long getValuation(long accountId)
    {
        return round(getCell(valuation, accountId));
    }
    ...
    private double getCell(double[] column, long accountId)
    {
        Integer index = accountId2Index.get(accountId);
        if (null == index)
        {
    ...
    private static void calculateValuation(double[] margin, double[] unrealisedPnL, double[] valuation)
    {
    ...
    private static void marginNotional(double[] notionalCost, double marginRate, double[] out)
    {
        for (int i = 0; i < out.length && i < notionalCost.length; i++)
        {
            out[i] = notionalCost[i] * marginRate;
        }
    }

    private static void unrealisedPnL(double[] notionalCost, long[] cumulativeCost, long[] realisedPnL, double[] out)
    {
        for (int i = 0; i < notionalCost.length && i < out.length; i++)
        {
            out[i] = notionalCost[i] - (cumulativeCost[i] + realisedPnL[i]);
        }
    }
